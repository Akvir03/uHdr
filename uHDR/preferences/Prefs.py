...
class Prefs:
    ...
    def load() -> None:
        with open(".\preferences\prefs.json") as f:
            ...
            if "extraPath" in allPrefs.keys():
                Prefs.extraPath = allPrefs["extraPath"]
            if "HDRdisplay" in allPrefs.keys():
                Prefs.HDRdisplay = allPrefs["HDRdisplay"]
            if "HDRdisplays" in allPrefs.keys():
                Prefs.HDRdisplays = allPrefs["HDRdisplays"]
            if "imgExt" in allPrefs.keys():
                Prefs.imgExt = allPrefs["imgExt"]
            if "thumbnailPrefix" in allPrefs.keys():
                Prefs.thumbnailPrefix = allPrefs["thumbnailPrefix"]
            if "thumbnailMaxSize" in allPrefs.keys():
                Prefs.thumbnailMaxSize = allPrefs["thumbnailMaxSize"]
            ...
            if "gui" in allPrefs.keys():
                if "gallerySize" in allPrefs["gui"].keys():
                    Prefs.gallerySize = tuple(allPrefs["gui"]["gallerySize"])
