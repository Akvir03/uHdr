...
class SelectionMap:
    ...
    def __repr__(self: SelectionMap) -> str:
        ...
        res += "--------------------------------------------------\n"
        res += (
            f"number of selected image:{self.numberSelectedImages}/{self.numberImages}"
            + "\n"
        )
        res += "--------------------------------------------------\n"
    ...
    def applySelection(self: SelectionMap, selection: list[tuple[str, bool]]) -> None:
        ...
        if debug:
            print(f"SelectionMap.applySelection({selection}):\n{self}")

    # ---------------------------------------------------------------------------------------
    def isSelected(self: SelectionMap, name: str) -> bool | None:
        if name in self.__imageNameToIsSelected.keys():
            return self.__imageNameToIsSelected[name]
        else:
            print(
                f'[ERROR] SelectionMap.isSelected({name}): imagefile "{name}" not found > return None'
            )
            return None
    ...
    def imageNameToGlobalIndex(self: SelectionMap, name: str) -> int | None:
        ...
        if name in self.__imageNameToGlobalIndex.keys():
            return self.__imageNameToGlobalIndex[name]
        else:
            print(
                f'[ERROR] SelectionMap.imageNameToGlobalIndex({name}): imagefile "{name}" not found > return None'
            )
            return None

    # ---------------------------------------------------------------------------------------
    def globalIndexToImageName(self: SelectionMap, gIdx: int) -> str | None:
        if gIdx in self.__globalIndexToImageName.keys():
            return self.__globalIndexToImageName[gIdx]
        else:
            print(
                f"[ERROR] SelectionMap.globalIndexToImageName({gIdx}): key ({gIdx}) not found > return None"
            )
            return None

    # ---------------------------------------------------------------------------------------
    def globalIndexToSelectedIndex(self: SelectionMap, gIdx: int) -> int | None:
        if gIdx in self.__globalIndexToSelectedIndex.keys():
            return self.__globalIndexToSelectedIndex[gIdx]
        else:
            return None

    # ---------------------------------------------------------------------------------------
    def selectedlIndexToGlobalIndex(self: SelectionMap, sIdx: int) -> int | None:
        if sIdx in self.__selectedIndexToGlobalIndex.keys():
            return self.__selectedIndexToGlobalIndex[sIdx]
        else:
            return None
    ...
    def selectedIndexToImageName(self: SelectionMap, sIdx: int) -> str | None:
        ...
        if gIdx != None:
            return self.globalIndexToImageName(gIdx)
        else:
            return None
    ...
    def imageNameToSelectedIndex(self: SelectionMap, name: str) -> int | None:
        ...
        if gIdx != None:
            return self.globalIndexToSelectedIndex(gIdx)
        else:
            return None

    # ---------------------------------------------------------------------------------------
    def selectByScore(
        self: SelectionMap, scores: dict[str, int], scoresSelected: list[int]
    ):
    ...
    # ---------------------------------------------------------------------------------------
    def getSelectedImageNumber(self: SelectionMap) -> int:
        return self.numberSelectedImages
