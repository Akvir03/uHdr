...
class ImageFiles(QObject):
    ...
    def setDirectory(self: ImageFiles, dirPath: str) -> int:
        ...
        if debug:
            print(f"ImageFiles.setDirectory({dirPath})")
        ...
        for filename in self.imageFilenames:
            # tags
            self.imageTags[filename] = Tags.load(
                self.imagePath, filename, self.extraPath
            )
            # jexif
            self.imageExif[filename] = Jexif.load(
                self.imagePath, filename, self.extraPath
            )
            # score
            self.imageScore[filename] = Score.load(
                self.imagePath, filename, self.extraPath
            )
    ...
    def requestLoad(self: ImageFiles, filename: str, thumbnail: bool = True):
        ...
        if debug:
            print(f"ImageFiles.requestLoad({filename}, thumbnail={thumbnail})")
    ...
    def endLoadImage(self: ImageFiles, error: bool, filename: str):
        ...
        if debug:
            print(f"ImageFiles.endLoadImage( error={error}, {filename})")
    ...
    def updateImageScore(self: ImageFiles, imageName: str, value: int) -> None:
        ...
        Score.save(
            self.imagePath, self.extraPath, imageName, self.imageScore[imageName]
        )
...
class RunLoadImage(QRunnable):
    def __init__(
        self: RunLoadImage, parent: ImageFiles, filename: str, thumbnail: bool = True
    ):
    ...
    def run(self: RunLoadImage):
        ...
        if debug:
            print(f"RunLoadImage.run({self.filename})")
        ...
        try:
            ...
            if os.path.exists(self.filename):
                ...
                if self.thumbnail:  # thumbnail not original image
                    ...
                    thumbnailName: str = os.path.join(
                        path, Prefs.extraPath, Prefs.thumbnailPrefix + name + "." + ext
                    )
                    ...
                    if os.path.exists(thumbnailName):
                        ...
                        imageSmall: Image = imageBig.buildThumbnail(
                            Prefs.thumbnailMaxSize
                        )
