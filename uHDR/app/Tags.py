...
class Tags:
    ...
    def save(self: Self, imageDir: str, extraDir: str, imageFilename: str) -> None:
        if debug:
            print(f"app.Tags.save({imageDir},{extraDir},{imageFilename})")
        # check if extra '.uHDR' dir exist
        extraPath = os.path.join(imageDir, extraDir)
        if os.path.isdir(extraPath):
            tagsFilename = os.path.join(imageDir, extraDir, imageFilename[:-3] + "tags")
            with open(tagsFilename, "w") as tagsfile:
                if debug:
                    print(
                        f"app.Tags.save({imageDir},{extraDir},{imageFilename}) > {self.tags}"
                    )
    ...
    def add(self: Self, type: str, name: str, value: bool) -> None:
        if debug:
            print(f"app.Tags.add({type},{name},{value})")

        if type in self.tags.keys():
            self.tags[type][name] = value
        else:
            self.tags[type] = {name: value}
    ...
    ## -------------------------------------------------------
    @staticmethod
    def aggregateTagsFiles(
        imagePath: str, extraPath: str
    ...
    ## -------------------------------------------------------
    @staticmethod
    def aggregateTagsData(
        tags: list[dict[str, dict[str, bool]]]
    ) -> dict[str, dict[str, bool]]:
