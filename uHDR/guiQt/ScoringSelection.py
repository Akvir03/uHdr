...
class ScoringSelection(QFrame):
    ...
    def setSelection(self: Self, selection: list[bool]) -> None:
        assert len(selection) == len(
            self.score
        ), f"ScoringSelection.setSlection({selection}) len ERROR"
        self.active = False
        for i, cb in enumerate(self.checkBoxes):
    ...
    def CBscoreChanged(self: Self, idx: int) -> None:
        if debug:
            print(f"ScoringSelection.CBscoreChanged({idx}) >actice {self.active} ")

        if self.active:
            self.score[idx] = self.checkBoxes[idx].isChecked()
            if debug:
                print(f"\tScoringSelection.CBscoreChanged({idx}) >emit {self.score} ")
