...
class CurveWidget(QFrame):
    ...
    def __init__(self: Self) -> None:
        ...
        self.default: dict[str, list[float]] = {
            "start": [0.0, 0.0],
            "shadows": [10.0, 10.0],
            "blacks": [30.0, 30.0],
            "mediums": [50.0, 50.0],
        ...
        self.curveWidget.plot(
            np.asarray([0.0, 100]), np.asarray([0.0, 100.0]), "r--", clear=True
        )
        ...
        self.shadows: AdvanceSliderLine = AdvanceSliderLine(
            "shadows", self.default["shadows"][1], (0, 100), (0, 100), 10
        )
        self.blacks: AdvanceSliderLine = AdvanceSliderLine(
            "blacks", self.default["blacks"][1], (0, 100), (0, 100), 10
        )
        self.mediums: AdvanceSliderLine = AdvanceSliderLine(
            "mediums", self.default["mediums"][1], (0, 100), (0, 100), 10
        )
        self.whites: AdvanceSliderLine = AdvanceSliderLine(
            "whites", self.default["whites"][1], (0, 100), (0, 100), 10
        )
        self.highlights: AdvanceSliderLine = AdvanceSliderLine(
            "highlights", self.default["highlights"][1], (0, 100), (0, 100), 10
        )
    ...
    ## setValue
    def setKey(
        self: Self, key: str, value: int | float, autoScale: bool = False
    ) -> None:
        value = float(value)
        ...
        if key in self.control.keys():
            ...
            if (listValues[:index, 1] <= value).all() and (
                value <= listValues[index + 1 :, 1]
            ).all():
                # can change
                ...
                if autoScale:
                    ...
                    for i, v in enumerate(newValues):
                        ...
                        self.control[listKeys[i + index + 1]] = [
                            oldValue[0],
                            np.round(v),
                        ]
                else:
    ...
    def evaluate(self: Self) -> None:
        self.curve.ctrlpts = copy.deepcopy(
            [
                self.control["start"],
                self.control["shadows"],
                self.control["blacks"],
                self.control["mediums"],
                self.control["whites"],
                self.control["highlights"],
                [200, self.control["end"][1]],
            ]
        )
    ...
    def plotCurve(self):
        try:
            self.curveWidget.plot(
                np.asarray([0, 100]), np.asarray([0, 100]), "r--", clear=True
            )
            self.curveWidget.plot(
                np.asarray([20, 20]), np.asarray([0, 100]), "r--", clear=False
            )
            self.curveWidget.plot(
                np.asarray([40, 40]), np.asarray([0, 100]), "r--", clear=False
            )
            self.curveWidget.plot(
                np.asarray([60, 60]), np.asarray([0, 100]), "r--", clear=False
            )
            self.curveWidget.plot(
                np.asarray([80, 80]), np.asarray([0, 100]), "r--", clear=False
            )

            controlPointCoordinates = np.asarray(self.curve.ctrlpts)
            self.curveWidget.plot(
                controlPointCoordinates[1:-1, 0],
                controlPointCoordinates[1:-1, 1],
                "ro",
                clear=False,
            )
            if isinstance(self.points, ndarray):
                x = self.points[:, 0]
                self.curveWidget.plot(
                    self.points[x < 100, 0], self.points[x < 100, 1], "r", clear=False
                )
        except:
