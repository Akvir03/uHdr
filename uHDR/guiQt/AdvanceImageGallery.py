...
class AdvanceImageGallery(QSplitter):
    ...
    def __init__(self: AdvanceImageGallery, nbImages: int = 0) -> None:
        ...
        self.navLayout: QHBoxLayout = QHBoxLayout()
        self.navContainer.setLayout(self.navLayout)
        self.previousPage: QPushButton = QPushButton("< previous")
        self.navLayout.addWidget(self.previousPage)
        self.navLayout.addStretch()
        self.one: QPushButton = QPushButton("[    ]")
        self.navLayout.addWidget(self.one)
        self.zoomLabel: QLabel = QLabel("zoom:")
        self.navLayout.addWidget(self.zoomLabel)
        self.zoomMinus: QPushButton = QPushButton("(+)")
        self.navLayout.addWidget(self.zoomMinus)
        ...
        self.zoomPlus: QPushButton = QPushButton("(-)")
        self.navLayout.addWidget(self.zoomPlus)
        self.navLayout.addStretch()
        self.pageLabel: QLabel = QLabel("page: X/XX")
        self.navLayout.addWidget(self.pageLabel)
        self.navLayout.addStretch()
        self.nextPage: QPushButton = QPushButton("next >")
        self.navLayout.addWidget(self.nextPage)
    ...
    def zoomSliderChanged(self: AdvanceImageGallery):
        ...
        if debug:
            print(f"AdvanceImageGallery.zoomSliderChanged() > active: {self.active}")

        if self.active:
            if self.zoomSlider.value() > self.size[1]:
                while self.zoomSlider.value() > self.size[1]:
                    self.incSize()
    ...
    def setSize(self: AdvanceImageGallery, size: tuple[int, int]) -> None:
        ...
        if debug:
            print(
                f"AdvanceImageGallery.setSize({size}) > emit requestImage({self.getImageRangeIndex()})"
            )
    ...
    ## pages management
    @property
    def nbImgPerPage(self: AdvanceImageGallery) -> int:
    ...
    def CBOne(self: AdvanceImageGallery) -> None:
        ...
        if debug:
            print(f"AdvanceImageGallery.CBOne():")
        ...
        if self.size == (1, 1):
            ...
            if debug:
                print(
                    f"AdvanceImageGallery.CBOne(): > return to previous size: {self.previousSize}"
                )
            ...
            if self.selectedImage != None:
                if debug:
                    print(
                        f"AdvanceImageGallery.CBOne(): > goto one page, index: {self.selectedImage}"
                    )
                ...
                if debug:
                    print(
                        f"\tAdvanceImageGallery.CBOne(): > goto one page, index: {self.selectedImage} > requestImages.emit({self.selectedImage}, {self.selectedImage})"
                    )  #####
                ...
                if debug:
                    print(
                        f"AdvanceImageGallery.CBOne(): > goto one page, index: {self.imageLocalIdxToGlobalIndex(0)}"
                    )
    ...
    def firstPage(self: AdvanceImageGallery) -> None:
        ...
        if debug:
            print(f"AdvanceImageGallery.firstPage():")
    ...
    def CBnextPage(self: AdvanceImageGallery) -> None:
        if debug:
            print(f"AdvanceImageGallery.CBnextPage():")

        if self.nbImages > 0:
            self.pageIndex = (self.pageIndex + 1) % ceil(
                self.nbImages / self.nbImgPerPage
            )
            self.updatePageInfo()
    ...
    def CBpreviousPage(self: AdvanceImageGallery) -> None:
        if debug:
            print(f"AdvanceImageGallery.CBpreviousPage():")
    ...
    def imageIdxToIndexInPage(self: AdvanceImageGallery, imageIndex: int) -> int:
        ...
        if debug:
            print(
                f"AdvanceImageGallery.imageIdxToIndexInPage({imageIndex}) > {imageIndex}%{self.nbImgPerPage} >> {imageIndex%self.nbImgPerPage}"
            )
    ...
    def imageIdxToPageIndex(self: AdvanceImageGallery, imageIndex: int) -> int:
        ...
        if debug:
            print(
                f"AdvanceImageGallery.imageIdxToPageIndex({imageIndex}) > {imageIndex}//{self.nbImgPerPage} >> {imageIndex//self.nbImgPerPage}"
            )
    ...
    def sendRequestImages(self: AdvanceImageGallery) -> None:
        if debug:
            print(f"AdvanceImageGallery.sendRequestImages()")
        self.requestImages.emit(*self.getImageRangeIndex())
    ...
    def setImage(self: AdvanceImageGallery, index: int, image: ndarray | None) -> None:
        ...
        if debug:
            print(
                f"AdvanceImageGallery.setImage({index}) > {self.imageIdxToIndexInPage(index)} "
            )
    ...
    def CBimageSelected(self: AdvanceImageGallery, idxIW: int) -> None:
        ...
        if imgIdx < self.nbImages:
            ...
            if debug:
                print(
                    f"AdvanceImageGallery.CBimageSelected({idxIW}) -> image index:{self.selectedImage} "
                )
