...
class LchSelector(QFrame):
    ...
    def __init__(self: Self) -> None:
        ...
        ##### ch view
        self.chromaHue: ImageWidget = ImageWidget(np.ones((300, 300, 3)) * 0.5)
        self.chromaHue.setMinimumSize(200, 200)

        ### Lh view
        self.lightnessHue: ImageWidget = ImageWidget(np.ones((300, 300, 3)) * 0.5)
        ...
        ### hue
        hueBarLch: np.ndarray = colourData.buildLchcolourData(
            (75, 75), (100, 100), (0, 360 + 90), (20, 720), width="h", height="c"
        )
        hueBarRGB: np.ndarray = colourSpace.Lch_to_sRGB(
            hueBarLch, apply_cctf_encoding=True, clip=True
        )
        self.hueSelector: ChannelSelector = ChannelSelector(
            "hue", hueBarRGB, (0, 360 + 90), (0, 360 + 90)
        )

        ### chroma
        chromaBarLch: np.ndarray = colourData.buildLchcolourData(
            (75, 75), (0, 100), (180, 180), (20, 720), width="c", height="L"
        )
        chromaBarRGB: np.ndarray = colourSpace.Lch_to_sRGB(
            chromaBarLch, apply_cctf_encoding=True, clip=True
        )
        self.chromaSelector: ChannelSelector = ChannelSelector(
            "chroma", chromaBarRGB, (0, 100), (0, 100)
        )
    ...
    def CBhueSelectionChanged(self: Self) -> None:
        ...
        # compute chroma bar
        chromaBarLch: np.ndarray = colourData.buildLchcolourData(
            (75, 75), (0, 100), (hue, hue), (20, 720), width="c", height="L"
        )
        chromaBarRGB: np.ndarray = colourSpace.Lch_to_sRGB(
            chromaBarLch, apply_cctf_encoding=True, clip=True
        )
    ...
    def updateView(self: Self) -> None:
        # chrmaHue
        chromaHueLCh: np.ndarray = colourData.buildLchcolourData(
            (75, 75), self.chromaRange, self.hueRange, (200, 200), width="h", height="c"
        )
        chromaHueRGB: np.ndarray = colourSpace.Lch_to_sRGB(
            chromaHueLCh, apply_cctf_encoding=True, clip=True
        )
        self.chromaHue.setPixmap(chromaHueRGB)
        # chromaLightness
        chromaLightnessLch: np.ndarray = colourData.buildLchcolourData(
            self.LightnessRange,
            self.chromaRange,
            self.hueRange,
            (200, 200),
        ...
        chromaLightnessRGB: np.ndarray = colourSpace.Lch_to_sRGB(
            chromaLightnessLch, apply_cctf_encoding=True, clip=True
        )
