...
class InfoBase(QFrame):
    ...
    def toGui(self: Self) -> dict:
        ...
        res["Size (pixel): "] = (
            (
                ...
                + " x "
                + (str(self.size[1]) if self.size[1] != -1 else "...")
            ),
            False,
        )
        res["Color Space: "] = (
            self.colorSpace if self.colorSpace != "" else "...",
            False,
        )
        res["Type: "] = (self.type if self.type != "" else "...", False)
        res["Bit per sample: "] = (
            str(self.bitPerSample) if self.bitPerSample != -1 else "...",
            False,
        )
    ...
    def setInfo(
        ...
        if debug:
            print(
                f"InfoBase.setInfo({name}, {path} ,{size}, {colorSpace},{type},{bps})"
            )
        self.name = name
        self.path = path
        self.size = size
        ...
        self.baseInfo.setText(
            [
                self.name,
                self.path,
                (str(self.size[0]) if self.size[0] != -1 else "...")
                + " x "
                + (str(self.size[1]) if self.size[1] != -1 else "..."),
                self.colorSpace,
                self.type,
