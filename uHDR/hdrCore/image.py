...
class Image(object):
    ...
    def __init__(
        self, path, name, colorData, type, linear, colorspace, scalingFactor=1.0
    ):
        ...
        self.shape = (
            self.colorData.shape
        )  # image size height,width, color channel number (tuple)
    ...
    def read(filename, thumb=False):
        ...
        if ext == "arw":
            ...
            imgDouble = colour.utilities.as_float_array(raw.postprocess(ppParams)) / (
                pow(2, 16) - 1
            )
            raw.close()
        ...
        # post processing for HDR scaling to [ ,1]
            ...
            if thumb:
                ...
                if os.path.exists(searchStr):
                    imgDouble = colour.read_image(
                        searchStr, bit_depth="float32", method="Imageio"
                    )  # <--- read thumbnail of input file
                    ...
                    imgThumbnail = skimage.transform.resize(
                        imgDouble, (int(iY * factor), maxX)
                    )
                    # save thumbnail
        ...
        res = Image(
            path,
            name + "." + ext,
            np.float32(imgDouble),
            type,
            linear,
            None,
            scalingFactor,
        )  # colorspace = None will be set in metadata.metadata.build(res)
        res.metadata = metadata.metadata.build(
            res
        )  # build metadata (read if json file exists, else recover from exif data)
    ...
    def getDynamicRange(self, percentile=None):
        ...
        if percentile == None:
            Y_min, Y_max = np.amin(Y[Y > 0]), np.amax(Y)  # use min and max
        else:
            Y_min, Y_max = np.percentile(Y[Y > 0], percentile), np.percentile(
                Y, 100 - percentile
            )  # percentile
    ...
    def buildHistogram(self, channel):
        ...
        Args:
            channel: TODO
                TODO
        """
        self.histogram = Histogram.buildbuild(
            self, channel, nbBins=100, range=None, logSpace=self.isHDR()
        )
    ...
    def plot(self, ax, displayTitle=False, title=None, forceToneMapping=True, TMO=None):
        ...
        if not title:
            title = (
                self.name + "(" + self.colorSpace.name + " | " + str(self.type) + ")"
            )
        if not TMO:
            TMO = processing.tmo_cctf()
    ...
    def split(self, widthSegment, heightSegment):
        ...
        widthLimit = [
            (i * (imageWidth // widthSegment)) for i in range(widthSegment)
        ] + [imageWidth]
        heightLimit = [
            (i * (imageHeight // heightSegment)) for i in range(widthSegment)
        ] + [imageHeight]
        ...
        for line in range(heightSegment):
            ...
            for col in range(widthSegment):
                cData = copy.deepcopy(
                    self.colorData[
                        (heightLimit[line]) : (heightLimit[line + 1]),
                        (widthLimit[col]) : (widthLimit[col + 1]),
                        :,
                    ]
                )
                imgTemp = Image(
    ...
    def merge(imgList):
        ...
        Returns:
            (hdrCore.image.Image)
        """
        totalWidth = functools.reduce(
            lambda x, y: x + y, map(lambda img: img.colorData.shape[1], imgList[0]), 0
        )
        totalHeight = functools.reduce(
            lambda x, y: x + y, map(lambda imgList: imgList[0].shape[0], imgList), 0
        )
        ...
        return Image(
            copy.deepcopy(imgList[0][0].path),
            copy.deepcopy(imgList[0][0].name),
            cData,
            copy.deepcopy(imgList[0][0].type),
            imgList[0][0].linear,
            copy.deepcopy(imgList[0][0].colorSpace),
            imgList[0][0].scalingFactor,
        )
...
class Histogram(object):
    ...
    def __repr__(self):
        ...
        res = (
            ...
            + "\n"
            + "\t nb bins: "
            + str(len(self.histValue))
            + "\n"
            + "\t channel: "
            + str(self.channel.name)
            + "\n"
            + "\t logSpace: "
            + str(self.logSpace)
            + ">"
    ...
    def build(img, channel, nbBins=100, range=None, logSpace=None):
        ...
        """
        # logSpace
        if not isinstance(logSpace, (bool, str)):
            logSpace = "auto"
        ...
        if logSpace:
            ...
            # bins
            bins = 10 ** np.linspace(
                np.log10(minChannel), np.log10(maxChannel), nbBins + 1
            )
        else:
    ...
    def plot(self, ax, color="r", shortName=True, title=True):
        ...
        if self.logSpace:
            ax.set_xscale("log")
        name = (
            self.name.split("/")[-1] + "(H(" + self.channel.name + "))"
            if shortName
            else self.name + "(Histogram:" + self.channel.name + ")"
        )
        if title:
            ax.set_title(name)
