...
class tags:
    ...
    def __init__(self):
        if os.path.exists("./preferences/tags.json"):
            with open("./preferences/tags.json") as f:
...
class metadata:
    ...
    def __init__(self, _image):
        ...
        self.otherTags = tags()
        self.metadata[self.otherTags.getTagsRootName()] = copy.deepcopy(
            self.otherTags.tags[self.otherTags.getTagsRootName()]
        )
                ...
                if pref.keepAllMeta:
                    ...
                    for keyInFile in metaInFile.keys():
                        if keyInFile in res.metadata:
                            res.metadata[keyInFile] = copy.deepcopy(
                                metaInFile[keyInFile]
                            )
                        else:
                            print(
    ...
    def readExif(filename):
        ...
        if os.path.isfile(filename):  # check if filename exists
            if os.path.isfile("exiftool.exe"):  # reading metadata with exiftool
                try:
                    exifdata = subprocess.check_output(
                        ["exiftool.exe", "-a", filename],
                        shell=True,
                    ...
                    print(
                        "ERROR[metadata.readExif(", filename, "): error while reading!]"
                    )
                    exifdata = []
                # buid exif dict
                for each in exifdata:
                    tag, val = each.split(
                        ":", 1
                    )  # tags and values are separated by a semi colon
                    exifDict[tag.strip()] = val.strip()
            else:
                print(
                    "ERROR[metadata.readExif(",
                    filename,
                    "): consider installing exiftool for better exif metadata, degraded mode with imageio!]",
                )
                img = imageio.imread(filename)
                exifDict = img.meta["EXIF_MAIN"] if "EXIF_MAIN" in img.meta else {}
        else:
            print("ERROR[metadata.readExif(", filename, "): file not found]")
    ...
    def recoverData(self, exif):
        ...
        if not (exif == {}):  # exif not empty
            ...
            if exifColorSpace[0] in exif:
                self.metadata["exif"]["Color Space"] = exif[exifColorSpace[0]]
            elif exifColorSpace[1] in exif:
                self.metadata["exif"]["Color Space"] = exif[exifColorSpace[1]]
            elif exifColorSpace[2] in exif:
                if exif[exifColorSpace[2]] == 1:
                    self.metadata["exif"]["Color Space"] = "sRGB"
                elif exif[exifColorSpace[2]] == 2:
                    self.metadata["exif"]["Color Space"] = "Adobe RGB (1998)"
                else:
                    self.metadata["exif"][
                        "Color Space"
                    ] = metadata.defaultColorSpaceName
            else:
                print(
                    "WARNING[",
                    self.image.name,
                    "exif does not contain 'Color Space' nor 'Profile Description', color space is set to sRGB  or scRGB for HDR images!]",
                )
            ...
            elif exifEposureTime[1] in exif:
                self.metadata["exif"]["Exposure Time"] = exif[exifEposureTime[1]]
            else:
                self.metadata["exif"]["Exposure Time"] = None
            ...
            if exifAperture[0] in exif:
                ...
                if "/" in apertureValue:
                    ...
                    self.metadata["exif"]["F Number"] = (
                        ast.literal_eval(apertureValue),
                        1,
                    )
            elif exifEposureTime[1] in exif:
                self.metadata["exif"]["F Number"] = exif[exifAperture[1]]
            else:
                self.metadata["exif"]["F Number"] = None
            ...
            elif exifISO[1] in exif:
                self.metadata["exif"]["ISO"] = exif[exifISO[1]]
            else:
                self.metadata["exif"]["ISO"] = None

            # Bits Per Sample: 'Bits Per Sample'
            exifBPS = ["Bits Per Sample"]
            if exifBPS[0] in exif:
                self.metadata["exif"]["Bits Per Sample"] = ast.literal_eval(
                    exif[exifBPS[0]]
                )
            else:
                self.metadata["exif"]["Bits Per Sample"] = None
            ...
            if exifCamera[0] in exif:
                ...
                if exifCamera[1] in exif:
                    cameraMakeModel += " " + exif[exifCamera[1]]
                elif exifCamera[2] in exif:
                    cameraMakeModel += " " + exif[exifCamera[2]]
            else:
                cameraMakeModel = None
            self.metadata["exif"]["Camera"] = cameraMakeModel

            # Software: 'Software'
            exifSoftware = ["Software"]
            if exifSoftware[0] in exif:
                self.metadata["exif"]["Software"] = exif[exifSoftware[0]]
            else:
                self.metadata["exif"]["Software"] = None

            # Lens: 'Lens Model', 'LensModel'
            exifLens = ["Lens Model", "LensModel"]
            if exifLens[0] in exif:
                self.metadata["exif"]["Lens"] = exif[exifLens[0]]
            elif exifLens[1] in exif:
                self.metadata["exif"]["Lens"] = exif[exifLens[1]]
            else:
                self.metadata["exif"]["Lens"] = None
            ...
            elif exifFocalLength[1] in exif:
                self.metadata["exif"]["Focal Length"] = exif[exifFocalLength[1]]
            else:
                self.metadata["exif"]["Focal Length"] = None

        else:  # exif data = {}
            self.metadata["exif"]["Color Space"] = metadata.defaultColorSpaceName
            print(
                " [META] >> metadata.recoverData(",
                self.image.name,
                ").colorSpace undefined set it to:",
                "sRGB or scRGB for HDR images",
            )

        # bit per sample
        if self.metadata["exif"]["Bits Per Sample"] == None:
            if self.image.name.endswith(".hdr"):
                self.metadata["exif"]["Bits Per Sample"] = 32
            if self.image.name.endswith(".jpg"):
                self.metadata["exif"]["Bits Per Sample"] = 8
