...
class Exif:
    ...
    def readExif(imagePath: str, filename: str) -> dict[str, str] | None:
        ...
        if os.path.isfile(file):  # check if filename exists
            ...
            print("ERROR: Exif.readExif(", filename, "): file not found!")
    ...
    def recoverExifData(exif: dict[str, str]) -> dict[str, str]:
        ...
        if exifColorSpace[0] in exif.keys():
            filteredExif["Color Space"] = exif[exifColorSpace[0]]
        elif exifColorSpace[1] in exif.keys():
            filteredExif["Color Space"] = exif[exifColorSpace[1]]
        elif exifColorSpace[2] in exif.keys():
            if exif[exifColorSpace[2]] == 1:
                filteredExif["Color Space"] = "sRGB"
            elif exif[exifColorSpace[2]] == 2:
                filteredExif["Color Space"] = "Adobe RGB (1998)"
        elif "File Name" in exif.keys():
            filename: str = exif["File Name"]
            if filename.endswith((".hdr", ".HDR")):
                filteredExif["Color Space"] = "scRGB"
            elif filename.endswith((".jpg", ".JPG")):
                filteredExif["Color Space"] = "sRGB"

        # Bits Per Sample
        if "Bits Per Sample" in exif.keys():
            filteredExif["Bits Per Sample"] = exif["Bits Per Sample"]
        elif "File Name" in exif.keys():
            filename: str = exif["File Name"]
            if filename.endswith((".hdr", ".HDR")):
                filteredExif["Bits Per Sample"] = "32"
            elif filename.endswith((".jpg", ".JPG")):
                filteredExif["Bits Per Sample"] = "8"

        # Type
        if "File Name" in exif.keys():
            filename: str = exif["File Name"]
            if filename.endswith((".hdr", ".HDR")):
                filteredExif["Type"] = "HDR"
            elif filename.endswith((".jpg", ".JPG")):
                filteredExif["Type"] = "SDR"
