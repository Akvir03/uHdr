...
class Palette(ImageAestheticsModel):
    ...
    def __init__(self, name, colors, colorSpace, type):
        ...
        self.colors = np.asarray(
            sorted(colors.tolist(), key=lambda u: np.sqrt(np.dot(u, u)))
        )
        self.type = type
    ...
    def build(processpipe, nbColors=5, method="kmean-Lab", processId=-1, **kwargs):
        ...
        if method == "kmean-Lab":
            ...
            if "removeBlack" in kwargs:
                removeBlack = kwargs["removeBlack"]
            else:
                removeBlack = defaultParams["removeBlack"]
            ...
            if removeBlack:
                ...
                cluster_centers_Lab = np.delete(
                    cluster_centers_Lab, idxLmin, axis=0
                )  # remove min from cluster_centers_Lab
        ...
        return Palette(
            "Palette_" + image_.name, colors, image.ColorSpace.Lab(), image_.type
        )
    ...
    def createImageOfPalette(self, colorWidth=100):
        ...
        return image.Image(
            ".", self.name, img, image.imageType.SDR, False, image.ColorSpace.sRGB()
        )
...
class MultidimensionalImageAestheticsModel:
    ...
    def get(self, key):
        iam = None
        if key in self.imageAestheticsModels:
            iam = self.imageAestheticsModels[key]
        return iam
