...
class exposure(Processing):
    ...
    def compute(self, img, **kwargs):
        ...
        defaultEV = 0.0
        if not kwargs:
            kwargs = {"EV": defaultEV}  # default value
        ...
        if EV != defaultEV:
            ...
            if not res.linear:
                ...
                if pref.computation == "python":
                    ...
                    start = timer()
                    res.colorData = numbafun.numba_cctf_sRGB_decoding(
                        res.colorData
                    )  # encode to prime
                    res.linear = True
                    ...
                    start = timer()
                    res.colorData = numbafun.cuda_cctf_sRGB_decoding(
                        res.colorData
                    )  # encode to prime
                    res.linear = True
    ...
    def auto(self, img):
        ...
        if not img.linear:
            rgbLinear = colour.cctf_decoding(rgb, function="sRGB")
        else:
            rgbLinear = rgb
...
class contrast(Processing):
    ...
    def compute(self, img, **kwargs):
        ...
        if not kwargs:
            kwargs = {"contrast": defaultContrast}  # default value

        if "contrast" in kwargs:
            contrastValue = kwargs["contrast"]
        else:
            contrastValue = defaultContrast
        ...
        if contrastValue != defaultContrast:
            ...
            if img.linear:
                if pref.computation == "python":
                    start = timer()
                    res.colorData = colour.cctf_encoding(
                        res.colorData, function="sRGB"
                    )  # encode to prime
                    res.linear = False

                elif pref.computation == "numba":
                    start = timer()
                    res.colorData = numbafun.numba_cctf_sRGB_encoding(
                        res.colorData
                    )  # encode to prime
                    res.linear = False
                    ...
                    start = timer()
                    res.colorData = numbafun.cuda_cctf_sRGB_encoding(
                        res.colorData
                    )  # encode to prime
                    res.linear = False
            ...
            if contrastValue >= 0.0:
                scalingFactor = (
                    1 * (1 - contrastValue) + maxContrastFactor * contrastValue
                )
            else:
...
class ColorSpaceTransform(Processing):
    ...
    def compute(self, img, **kwargs):
        ...
        if not kwargs:
            print(
                "WARNING[Processing.ColorSpaceTransform(",
                img.name,
                "):",
                "no destination colour space >> return a copy of image]",
            )
        else:
            if not "dest" in kwargs:
                print(
                    ...
                    img.name,
                    "):",
                    "no 'dest' colour space >> return a copy of image]",
                )
                ...
                if kwargs["dest"] == "Lab":  # DEST: Lab
                    ...
                    if currentCS == "sRGB":  # sRGB -> Lab
                        ...
                        res.colorData, res.linear, res.colorSpace = (
                            Lab,
                            None,
                    ...
                    if currentCS == "Lab":  # Lab -> sRGB
                        ...
                        XYZ = colour.Lab_to_XYZ(
                            Lab, illuminant=np.array([0.3127, 0.329])
                        )
                        apply_cctf_encoding = (
                            False if img.type == image.imageType.HDR else True
                        )
                        sRGB = colour.colour.XYZ_to_sRGB(
                            XYZ,
                            illuminant=np.array([0.3127, 0.329]),
                            chromatic_adaptation_transform="CAT02",
                            apply_cctf_encoding=apply_cctf_encoding,
                        )
                        res.colorData, res.colorSpace, res.linear = (
                            sRGB,
                            image.ColorSpace.sRGB(),
                            not apply_cctf_encoding,
                        )
                    ...
                    if currentCS == "sRGB":  # sRGB to XYZ
                        apply_cctf_decoding = (
                            True
                            if (img.type == image.imageType.SDR) and (not img.linear)
                            else False
                        )
                        RGB = res.colorData
                        XYZ = colour.sRGB_to_XYZ(
...
class resize(Processing):
    ...
    def compute(self, img, size=(None, None), anti_aliasing=False):
        ...
        if nx and (not ny):
            factor = nx / x
            res.colorData = skimage.transform.resize(
                res.colorData, (int(y * factor), nx), anti_aliasing
            )
            res.shape = res.colorData.shape
        elif (not nx) and ny:
            factor = ny / y
            res.colorData = skimage.transform.resize(
                res.colorData, (ny, int(x * factor)), anti_aliasing
            )
            res.shape = res.colorData.shape
        elif nx and ny:
            res.colorData = skimage.transform.resize(
                res.colorData, (ny, nx), anti_aliasing
            )
            res.shape = res.colorData.shape
    ...
    def compute(self, img, **kwargs):
        ...
        if kwargs != defaultControlPoints:
            ...
            if Y.shape == FY.shape:
                ...
                # transform colorData
                res.colorData[:, :, 0] = (
                    res.colorData[:, :, 0] * colorDataFY / colorDataY
                )
                res.colorData[:, :, 1] = (
                    res.colorData[:, :, 1] * colorDataFY / colorDataY
                )
                res.colorData[:, :, 2] = (
                    res.colorData[:, :, 2] * colorDataFY / colorDataY
                )
...
class saturation(Processing):
    ...
    def compute(self, img, **kwargs):
        ...
        if not kwargs:
            kwargs = defaultValue  # default value
...
class colorEditor(Processing):
    ...
    def compute(self, img, **kwargs):
        ...
        if not kwargs:
            kwargs = defaultValue  # default value
        if not ("selection" in kwargs):
            kwargs["selection"] = defaultValue["selection"]
        if not ("tolerance" in kwargs):
            kwargs["tolerance"] = defaultValue["tolerance"]
        if not ("edit" in kwargs):
            kwargs["edit"] = defaultValue["edit"]
        if not ("mask" in kwargs):
            kwargs["mask"] = defaultValue["mask"]
        ...
        if kwargs != defaultValue:
            ...
            # selection mask
            hMin, hMax = (
                kwargs["selection"]["hue"]
                if "hue" in kwargs["selection"].keys()
                else defaultValue["selection"]["hue"]
            )
            cMin, cMax = (
                kwargs["selection"]["chroma"]
                if "chroma" in kwargs["selection"].keys()
                else defaultValue["selection"]["chroma"]
            )
            lMin, lMax = (
                kwargs["selection"]["lightness"]
                if "hue" in kwargs["selection"].keys()
                else defaultValue["selection"]["lightness"]
            )
            ...
            lightnessMask = utils.NPlinearWeightMask(
                colorDataLightness, lMin, lMax, lightTolerance
            )
            chromaMask = utils.NPlinearWeightMask(
                colorDataChroma, cMin, cMax, chromaTolerance
            )
            ...
            hueShift = (
                kwargs["edit"]["hue"]
                if "hue" in kwargs["edit"].keys()
                else defaultValue["edit"]["hue"]
            )
            if hueShift != 0.0:
                colorLCH[:, :, 2] = (
                    (colorLCH[:, :, 2] + hueShift) % 360
                ) * mask + colorLCH[:, :, 2] * compMask

            # saturation (in Lch)
            saturation = (
                kwargs["edit"]["saturation"]
                if "saturation" in kwargs["edit"].keys()
                else defaultValue["edit"]["saturation"]
            )
            if saturation != 0:
                gamma = (
                    1 / ((saturation / 25) + 1)
                    if saturation >= 0
                    else (-saturation / 25) + 1
                )
                colorLCH[:, :, 1] = (
                    np.power(colorLCH[:, :, 1] / 100, gamma) * 100 * mask
                    + colorLCH[:, :, 1] * compMask
                )

            # exposure (in RGB)
            ev = (
                kwargs["edit"]["exposure"]
                if "exposure" in kwargs["edit"].keys()
                else defaultValue["edit"]["exposure"]
            )
            if ev != 0.0:
            ...
            con = (
                kwargs["edit"]["contrast"]
                if "exposure" in kwargs["edit"].keys()
                else defaultValue["edit"]["contrast"]
            )
            if con != 0:
                con = con / 100
                ...
                if not isinstance(colorRGB, np.ndarray):
                    colorRGB = Lch_to_sRGB(
                        colorLCH, apply_cctf_encoding=True, clip=False
                    )
                else:
                    colorRGB = colour.cctf_encoding(colorRGB, function="sRGB")

                colorRGBcon = (colorRGB - pivot) * scalingFactor + pivot

                colorRGB[:, :, 0] = (
                    colorRGBcon[:, :, 0] * mask + colorRGB[:, :, 0] * compMask
                )
                colorRGB[:, :, 1] = (
                    colorRGBcon[:, :, 1] * mask + colorRGB[:, :, 1] * compMask
                )
                colorRGB[:, :, 2] = (
                    colorRGBcon[:, :, 2] * mask + colorRGB[:, :, 2] * compMask
                )
...
class lightnessMask(Processing):
    ...
    def compute(self, img, **kwargs):
        ...
        if not kwargs:
            kwargs = defaultMask  # default value
        ...
        if kwargs != defaultMask:
            ...
            for key in rangeMask.keys():
                if kwargs[key]:  # mask on
                    mask[
                        (colorDataY >= rangeMask[key][0] / 100)
                        * (colorDataY < rangeMask[key][1] / 100),
                        :,
                    ] = np.asarray(maskColor[key])
    ...
    def compute(self, img, **kwargs):
        ...
        if int(imgRatio * 1000) != int(ratio[0] / ratio[1] * 1000):
            if imgRatio < (ratio[0] / ratio[1]):
                ...
                res.colorData = res.colorData[
                    (ch - hh16x9 - up) : (ch + hh16x9 - up), :, :
                ]
                res.shape = res.colorData.shape
            else:
        ...
        if rotation != 0:
            res.colorData = skimage.transform.rotate(
                res.colorData, rotation, clip=False, resize=False
            )
            h, w, _ = res.colorData.shape
            hh, ww = utils.croppRotated(h, w, rotation)
            res.colorData = res.colorData[
                int(h / 2 - hh / 2) : int(h / 2 + hh / 2),
                int(w / 2 - ww / 2) : int(w / 2 + ww / 2),
                :,
            ]
            res.shape = res.colorData.shape
...
class ProcessPipe(object):
    ...
    class ProcessNode(object):
        ...
        def setParameters(self, paramDict):
            ...
            if pref.verbose:
                print(
                    " [PROCESS] >> ProcessNode.setParameters(",
                    self.name,
                    "):",
                    paramDict,
                )
    ...
    def append(self, process, paramDict=None, name=None):
        ...
        Returns:
            TODO
                TODO
        """
        if isinstance(process, Processing):
            process = ProcessPipe.ProcessNode(
                process, paramDict, name
            )  # encapsulate process in processNode
    ...
    def setImage(self, img):
        ...
        if not img.linear:
            ...
            if pref.computation == "python":
                start = timer()
                img.colorData = np.float32(
                    colour.cctf_decoding(img.colorData, function="sRGB")
                )
                img.linear = True
                ...
                start = timer()
                img.colorData = numbafun.cuda_cctf_sRGB_decoding(
                    img.colorData
                )  # encode to prime
                img.linear = True
        ...
        for processNode in self.processNodes:
            processNode.requireUpdate = True
    ...
    def getImage(self, toneMap=True):
        ...
        if isinstance(self.originalImage, image.Image):  # if pipe has an image
            ...
            if (not self.originalImage.linear) and self.__outputImage.linear:
                self.__outputImage.colorData = colour.cctf_encoding(
                    self.__outputImage.colorData, function="sRGB"
                )
                self.__outputImage.linear = False

                if pref.verbose:
                    print(
                        " [PROCESS] >> ProcessPipe.getImage(",
                        self.__outputImage.name,
                        ", toneMap:",
                        toneMap,
                        "): encode to sRGB !",
                    )
                ...
                if pref.verbose:
                    print(
                        ...
                        ", ,toneMap:",
                        toneMap,
                        "): tone map using cctf encoding !",
                    )

            elif (
                self.__outputImage.isHDR()
                and (not self.__outputImage.linear)
                and (not toneMap)
            ):
                self.__outputImage.colorData = colour.cctf_decoding(
                    self.__outputImage.colorData, function="sRGB"
                )
            ...
            if len(self.processNodes) > 0:
                ...
                if progress:
                    progress.showMessage(
                        "computing: " + self.processNodes[0].name + " start!"
                    )
                    progress.repaint()
                self.processNodes[0].condCompute(self.__inputImage)
                if progress:
                    progress.showMessage(
                        "computing: " + self.processNodes[0].name + " done!"
                    )
                    progress.repaint()
                # other nodes
                for i, processNode in enumerate(self.processNodes[1:]):
                    if progress:
                        progress.showMessage(
                            "computing: " + processNode.name + " start!"
                        )
                        progress.repaint()
                    processNode.condCompute(self.processNodes[i].outputImage)
                    if progress:
                        progress.showMessage(
                            "computing: " + processNode.name + " done!"
                        )
                        progress.repaint()
    ...
    def setParameters(self, id, paramDicts):
        ...
        for processNode in self.processNodes[id:]:
            processNode.requireUpdate = True
    ...
    def updateProcessPipeMetadata(self):
        ...
        if isinstance(self.originalImage, image.Image):
            self.originalImage.metadata.metadata["processpipe"] = copy.deepcopy(ppMeta)
        if isinstance(self.__inputImage, image.Image):
            self.__inputImage.metadata.metadata["processpipe"] = copy.deepcopy(ppMeta)
        if isinstance(self.__outputImage, image.Image):
            self.__outputImage.metadata.metadata["processpipe"] = copy.deepcopy(ppMeta)
    ...
    def updateUserMeta(self, tagRootName, meta):
        ...
        if isinstance(self.originalImage, image.Image):
            self.originalImage.metadata.metadata[tagRootName] = copy.deepcopy(meta)
        if isinstance(self.__inputImage, image.Image):
            self.__inputImage.metadata.metadata[tagRootName] = copy.deepcopy(meta)
        if isinstance(self.__outputImage, image.Image):
            self.__outputImage.metadata.metadata[tagRootName] = copy.deepcopy(meta)
    ...
    def export(self, dirName, size=None, to=None, progress=None):
        ...
        if size:
            img = img.process(resize(), size=(None, size[1]))
